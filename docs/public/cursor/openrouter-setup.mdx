...
Keep the API key in Claude-Mem's credential file, not in `settings.json`:
...
DeepSeek's disk context cache is automatic. Claude-Mem preserves the stable
observer prompt while retaining only the newest conversation turns, which
keeps the reusable prefix and prevents request history from growing without
bound.
